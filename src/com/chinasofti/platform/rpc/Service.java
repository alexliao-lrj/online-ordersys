...
public class Service<T> {
	 ...
	 * 服务器端绑定的服务对象列表
	 * */
	private static Hashtable bindservice = new Hashtable();
	 ...
	 * */
	@Deprecated
	public static <T> T lookup(Class<T> cl, String servername, String bindName) {
		// 尝试获取远程业务服务对象的本地代理
		try {
			// 创建远程业务代理处理器
			ServiceHandler handler = new ServiceHandler(servicePort, bindName,
					servername);
			// 获取远程业务服务对象本地代理
			T proxyObj = (T) Proxy.newProxyInstance(cl.getClassLoader(),
