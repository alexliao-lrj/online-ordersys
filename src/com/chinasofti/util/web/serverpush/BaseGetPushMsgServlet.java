...
public abstract class BaseGetPushMsgServlet extends HttpServlet {
	 ...
	 * @param request Servlet获取到的Http请求对象
	 * @param response Servlet获取到的Http相应对象
	 * @return 创建好的消息处理对象，供Servlet使用
	 * */
	public abstract MessageHandler setHandler(final HttpServletRequest request,final HttpServletResponse response);
	public void initMSGServer(final HttpServletRequest request,final HttpServletResponse response){}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 设置正确的请求字符集以防止出现乱码
				...
				initMSGServer(request, response);
				// 调用子类中实现的setHandler方法构建消息的处理对象
				MessageHandler handler=setHandler(request, response);				
