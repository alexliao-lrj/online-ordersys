...
/**
 * Servlet implementation class removeRecommendFoodServlet
 */
public class removeRecommendFoodServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public removeRecommendFoodServlet() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		FoodMapping mapping2 = MapperFactory
		...
		mapping.removeRecommend(id);
		response.setContentType("text/xml");
		
		ArrayList<RecommendInfo> recommendList = mapping.getAllRecommend();
    	ArrayList<FoodInfo> list = new ArrayList();//mapping.getAllFood((page - 1) * 5, 5);
		for (int i= 0;i < recommendList.size();i++){
			list.add(mapping2.getFoodById(recommendList.get(i).getFood_id()));
		}
		try {
