...
<script type="text/javascript">
	function loadData(page) {
		//alert("succeed"+page);
		xmlAjaxRequest("showUnfinishedOrders?page=" + page + "&time="
				+ Math.random(), "get", true, null, showData, null, null);
	...
	function buyOrder(id, pageIndex) {
		// body...
		var se = confirm("确定买单吗?");
		if (se == true) {
			xmlAjaxRequest("buyOrderServlet?id=" + id + "&pageIndex="
					+ pageIndex + "&time=" + Math.random(), "get", true, null,
					showData, null, null);
		}
	}
	...
	function showData(responseXml, obj) {
		...
		for ( var i = 0; i < orders.length; i++) {
			...
			var child = order.childNodes;
			for ( var j = 0; j < child.length; j++) {
				var element = child[j];
				if (element.nodeName == "id") {
					id = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "table_id") {
					table_id = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "starttime") {
					starttime = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "price") {
					price = element.childNodes[0].nodeValue;
				}
			}
			...
			html += "<h3>" + id + "</h3>";
			html += "<h4>" + table_id + "</h4>";
			html += "<h4>" + starttime + "</h4>";
			html += "<h4>" + price + "</h4>";
			html += "</li>";
			html += "<li class=\"bt-nn\">"
			html += "<h4>" + "<Button onclick=\"buyOrder(" + id + ","
					+ pageIndex + ")\">FINISH?</Button></h4>";
			html += "</li>";
...
<body onload="loadData(1)">
		...
		<div class="container" style="height:90px;">
			<div class="top-header" style="height:90px;">
				...
				<div
					...
					<h4>
						<form name="pushmsg" method="post" action="pushall"
							target="targetFrame">
							...
							<input type="image" name="submit" value="confirm"
								src="/userhead/u1.jpg">
						</form>
				...
				<div id="msj"
					style="position:relative; left:600px; top:-42px; background:#FFFFFF; font-size:20px; width:500px;height:50px;text-align:center;line-height:16px;overflow:hidden;"></div>
	...
	<div class="menu">
		<div class="container">
			...
			<div class="our-menu-grids" id="listbody"></div>
		</div>
	</div>
	<div id="cen">
