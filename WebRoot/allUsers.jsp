...
<script type="text/javascript">
	function loadData(page) {
		xmlAjaxRequest("showUser?page=" + page + "&time=" + Math.random(),
				"get", true, null, showData, null, null);

	}
	function removeUser(id, pageIndex) {
		// body...
		var se = confirm("确定删除吗?");
		if (se == true) {
			xmlAjaxRequest("removeUserServlet?id=" + id + "&pageIndex="
	...
	function showData(responseXml, obj) {
		...
		for ( var i = 0; i < users.length; i++) {
			...
			for ( var j = 0; j < child.length; j++) {
				...
				} else if (element.nodeName == "account") {
					uaccount = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "password") {
					upass = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "salary") {
					usalary = element.childNodes[0].nodeValue;
				} else if (element.nodeName == "phone") {
					uphone = element.childNodes[0].nodeValue;
			...
			html += "<li class=\"im-g\">"
					+ "<a href=\"#\">"
					+ "<img src=" + bpic + " " +
							"class=\"img-responsive\" alt=\"no picture\">"
					+ "</a>";
			html += "</li>";
			html += "<li class=\"data\">";
			html += "<h3>" + id + "</h3>";
			...
			html += "</li>";
			html += "<li class=\"bt-nn\">"
			html += "<h4>" + "<Button onclick=\"removeUser(" + id + ","
					+ pageIndex + ")\">DELETE</Button></h4>";
			html += "<h4>" + "<a href=\"changeUserServlet?id=" + id
					+ "\">MODIFY</a></h4>";
			html += "</li>";
...
<body onload="loadData(1)">
	...
	<br>
	<div id="cen1">
		<a class="hvr-shutter-in-horizontal button" href="addUser.jsp">ADD</a>
	</div>
	<br>
	<br>
	<!-- //menu-page -->
